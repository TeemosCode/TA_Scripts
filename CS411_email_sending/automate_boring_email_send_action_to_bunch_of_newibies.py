...
STUDENT_INFO_FILE = "cs411_Student_info.csv"
...
MY_NAME = "Roger Ho"
MY_UID = "[something]"
# Personally I would use an additional text file of my choice and read it in instead. Putting this here as reference
EMAIL_MESSAGE = """
Hi there,
...
class TAEmail:
    ...
    def format_string(self, *args):
        return eval(self.message_body)


if __name__ == "__main__":

    email_session = smtplib.SMTP(MY_EMAIL_PROVIDER, 587)
    email_session.starttls()
    email_session.login(MY_OWN_EMAIL_ADDRESS, EMAIL_PASSWORD)
    ...
    with open(STUDENT_INFO_FILE) as student_info:
        for line in student_info:
            ...
            work_email["From"] = TA_email.own_email_addr
            work_email["To"] = TA_email.student_email_addr
            work_email["Subject"] = TA_email.subject
            work_email.attach(MIMEText(TA_email.message_body, "plain"))

            message = work_email.as_string()
            print("Sending Email to Student ====> Name: {}, "
                  "email: {}, subject: {}, Team Number: {}".format(student_name,
                                                              TA_email.student_email_addr,
                                                              TA_email.subject,
                                                              student_team_number))
            email_session.sendmail(TA_email.own_email_addr, TA_email.student_email_addr, message)
            print("Email Sent!...")
